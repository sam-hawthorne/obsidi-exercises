...
public class FeedMediaController {
	...
	public ResponseEntity<Object> getImageMediaByPostKey(@PathVariable("postKey") String postkey) throws InvalidImageMetaDataException {
		...
		try {
			...
			LOGGER.error("Unable to find image metadata by post key, cause ={}", e.getMessage());
			return ResponseEntity.badRequest().body(e.getMessage());
		}
		return new ResponseEntity<>(imageMetaDataList, HttpStatus.OK);
	}
	
	@GetMapping(value = "/image/metadata/{imageID}")
	public ResponseEntity<Object> getImageMediaByImageId(@PathVariable("imageID") String imageID) {
		...
		try {
			...
			LOGGER.error("Unable to find image metadata by image ID, cause = {}", e.getMessage());
			return ResponseEntity.badRequest().body(e.getMessage());
		}
